...
describe("getAnthropicSamplingParameters", () => {
  ...
  test("detects Opus models that do not support sampling parameters", () => {
    expect(isAnthropicSamplingUnsupportedModel("anthropic/claude-opus-4-8")).toBe(true);
    expect(isAnthropicSamplingUnsupportedModel("claude-opus-4.7")).toBe(true);
    expect(isAnthropicSamplingUnsupportedModel("anthropic/claude-sonnet-4-6")).toBe(false);
  });

  test("omits sampling parameters for Opus models that reject them", () => {
    expect(
      getAnthropicSamplingParameters(
